...
export class __RouteRegistrar
{
    ...
    attribute(key, value) {
        ...
        const attributeKey = Arr.get(this._aliases, key, key)
    ...
    __call(method, ...parameters) {
        if (inArray(method, this._passthru)) {
            return this._registerRoute(method, ...parameters)
        }

        if (inArray(method, this._allowedAttributes)) {
            if (method === 'middleware') {
                return this.attribute(method, isArray(parameters[0]) ? parameters[0] : parameters)
            }

            return this.attribute(method, arrayKeyExists(0, parameters) ? parameters[0] : true)
        }
