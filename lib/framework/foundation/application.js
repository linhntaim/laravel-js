...
import {Registrar} from '../contracts/routing/registrar.js'
import {EventServiceProvider} from '../events/event-service-provider.js'
import {AbstractRouter} from '../routing/abstract-router.js'
import {RoutingServiceProvider} from '../routing/routing-service-provider.js'
...
export class Application extends ApplicationContract
{
    ...
    bootstrapWith(bootstrappers) {
        ...
        bootstrappers.forEach(bootstrapperClass => {
            this.make('events').dispatch(`bootstrapping: ${bootstrapperClass.name}`, [this])

            this.make(bootstrapperClass).bootstrap(this)

            this.make('events').dispatch(`bootstrapped: ${bootstrapperClass.name}`, [this])
        })
    }
    ...
    _registerCoreContainerAliases() {
        for (const [key, aliases] of Object.entries({
            ...
            // TODO:
            'router': [AbstractRouter, Registrar],
        })) {
